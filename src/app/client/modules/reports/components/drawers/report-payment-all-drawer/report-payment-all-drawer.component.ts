...
@Component({
  selector: 'mib-report-payment-all-drawer',
  standalone: true,
  imports: [CommonModule, ReportDrawerLayoutComponent, ReportPeriodDebtorFormComponent, ReactiveFormsModule],
  templateUrl: './report-payment-all-drawer.component.html',
  styleUrls: ['./report-payment-all-drawer.component.scss']
})
export class ReportPaymentAllDrawerComponent implements OnInit {
  ...
  onReport() {
    ...
    this.reportService.paymentAll({ ...this.form.getRawValue() }).pipe(
      tap(data => {
