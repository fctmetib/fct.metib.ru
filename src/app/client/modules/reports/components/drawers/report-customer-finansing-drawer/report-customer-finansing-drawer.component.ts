...
import {
  ReportService, reportByAggregateViewTableConfig
...
@Component({
  selector: 'mib-report-customer-finansing-drawer',
  standalone: true,
  imports: [CommonModule, ReportDrawerLayoutComponent, ReportPeriodDebtorFormComponent, ReactiveFormsModule],
  templateUrl: './report-customer-finansing-drawer.component.html',
  styleUrls: ['./report-customer-finansing-drawer.component.scss']
})
export class ReportCustomerFinansingDrawerComponent implements OnInit {
  ...
  onReport() {
    ...
    this.reportService.customerFinansing({ ...this.form.getRawValue() }).pipe(
      tap(data => {
