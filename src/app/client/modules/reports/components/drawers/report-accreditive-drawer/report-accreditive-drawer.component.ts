...
import {
  reportByAccreditiveViewTableConfig,
...
@Component({
  selector: 'mib-report-accreditive-drawer',
  standalone: true,
  imports: [CommonModule, ReportDrawerLayoutComponent, ReportPeriodDebtorFormComponent, ReactiveFormsModule],
  templateUrl: './report-accreditive-drawer.component.html',
  styleUrls: ['./report-accreditive-drawer.component.scss']
})
export class ReportAccreditiveDrawerComponent implements OnInit {
  ...
  onReport() {
    ...
    this.reportService.accreditive({ ...this.form.getRawValue() }).pipe(
      tap(data => {
        this.reportViewPageService.navigateAndSetupTable({
          tableName: this.data.title,
          reportType: this.data.type,
          config: reportByAccreditiveViewTableConfig,
          data
        })
      }),
      finalize(() => {
