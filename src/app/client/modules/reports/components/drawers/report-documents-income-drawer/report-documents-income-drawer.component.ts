...
@Component({
  selector: 'mib-report-documents-income-drawer',
  standalone: true,
  imports: [CommonModule, ReportDrawerLayoutComponent, ReportPeriodDebtorFormComponent, ReactiveFormsModule],
  templateUrl: './report-documents-income-drawer.component.html',
  styleUrls: ['./report-documents-income-drawer.component.scss']
})
export class ReportDocumentsIncomeDrawerComponent implements OnInit {
  ...
  onReport() {
    ...
    this.reportService.documentsIncome({ ...this.form.getRawValue() }).pipe(
      tap(data => {
