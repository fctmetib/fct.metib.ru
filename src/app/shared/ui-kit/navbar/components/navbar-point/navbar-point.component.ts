...
export class NavbarPointComponent implements AfterViewInit, OnDestroy {
	...
	private observeIcons() {
		if (this.leftIcon) {
			this.mutationObserver.observe(this.leftIcon.nativeElement, {
				childList: true
			})
		}
	}
	...
	private updateClasses() {
		this.classes = {
			...
			'navbar-point_left-iconly': Boolean(
				this.leftIcon?.nativeElement?.querySelector('[navbar-point-left-icon]')
			),
