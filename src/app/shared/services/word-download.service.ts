...
export class WordDownloadService {
  ...
  public downloadDocWithTextFormatted(text: string, name: string = 'requisites'): void {
    const lines = text.split('\n').filter(line => line.trim() !== '');

    const paragraphs = lines.map(line => {
      const [key, ...rest] = line.split(':');
      if (rest.length > 0) {
        // Если строка содержит ключ и значение
        ...
        // Если строка просто текст (например, пример назначения платежа)
        return new Paragraph({
          text: line,
          spacing: { before: 200 }
        });
