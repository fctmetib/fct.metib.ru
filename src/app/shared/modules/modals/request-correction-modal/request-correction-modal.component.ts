...
export class RequestCorrectionModalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.totalSum = this.shipments.reduce((sum, current) => sum + current.Summ, 0);

    this.shipmentsCorrections.valueChanges.pipe(
      startWith(null),
      tap(() => this.calculateSums()),
      takeUntil(this.au.destroyer)
    ).subscribe();

    this.paginatorItemsPerPageControl.valueChanges.pipe(
      startWith(null),
  ...
  send() {
    ...
    this.requestsService.correction({
  ...
  private calculateSums() {
    this.totalSumToFactor = this.shipmentsCorrections.controls.reduce((sum, current) => sum + current.get('Amount').value, 0);
  }
