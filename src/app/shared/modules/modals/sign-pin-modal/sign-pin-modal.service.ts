...
export class SignPinModalService {
	...
	open<ActionType = any>(data: SignPinModalData<ActionType>): SignPinModalRef<ActionType> {
		return this.dialog.open(SignPinModalComponent, modalConfig(432, data))
	}

  sign<ActionType>(req$: Observable<ActionType>, loading$: BehaviorSubject<boolean>): Observable<SignPinModalOutput<ActionType>> {
